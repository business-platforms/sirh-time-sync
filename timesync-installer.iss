...
; Create external version file explicitly
...
function CreateVersionFile(AppDir: String; Version: String): Boolean;
var
  VersionFilePath: String;
  VersionContent: AnsiString;
begin
  Result := False;
  try
    VersionFilePath := AppDir + '\version.txt';
    VersionContent := Version;
    
    if SaveStringToFile(VersionFilePath, VersionContent, False) then
    begin
      Log('Successfully created version file: ' + VersionFilePath + ' with version: ' + Version);
      Result := True;
    end
    else
    begin
      Log('Failed to create version file: ' + VersionFilePath);
    end;
  except
    Log('Exception occurred while creating version file');
  end;
end;
...
begin
  ...
  begin
    ...
    // *** CRITICAL FIX: Create external version file ***
    CurrentVersion := '{#AppVersion}';
    if CreateVersionFile(AppDir, CurrentVersion) then
    begin
      Log('External version file created successfully');
    end
    else
    begin
      Log('Failed to create external version file - this may cause update issues');
    end;
    ...
    LogContent := 'Installation completed successfully' + #13#10 +
                  ...
                  'Install Path: ' + AppDir + #13#10 +
                  'Installation Time: ' + InstallationStartTime + #13#10 +
                  'Completion Time: ' + GetDateTimeString('yyyy-mm-dd hh:nn:ss', #0, #0) + #13#10 +
                  'Version file created: ' + AppDir + '\version.txt';
